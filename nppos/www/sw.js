// NPPOS Service Worker (production, served at /sw.js)
...
//
// Cache Strategy:
//   * Built app assets (/assets/nppos/nppos/): Cache First
//   * Navigations (/nppos*): Network First with offline fallback to app shell
//   * API calls (/api/*): Network ONLY. When offline they are BLOCKED and
//     return 503 {offline:true} — the app layer warns the user and reads from
//     the local Dexie database instead. We deliberately do NOT cache API
//     responses so we never serve stale business data.
...
self.addEventListener("fetch", (event) => {
  ...
  // payload. These are never cached so no stale data can leak into the app.
